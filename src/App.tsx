...
function App() {
  ...
  const [user, setUser] = useState<{ id: number; name: string; username: string; email: string; role: string; points: number; avatarUrl?: string } | null>(null);
  const [email, setEmail] = useState("");
  const [profileName, setProfileName] = useState("");
  const [profileUsername, setProfileUsername] = useState("");
  const [profileEmail, setProfileEmail] = useState("");
  const [profileAvatarUrl, setProfileAvatarUrl] = useState("/default-avatar.svg");
  const handleAvatarFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // In dev mode the backend serves uploads on port 4000. Use absolute URL to avoid proxy issues.
      const avatarPath = data.avatarUrl;
      const serverOrigin = `${window.location.protocol}//${window.location.hostname}:4000`;
      const finalUrl = avatarPath && avatarPath.startsWith("/uploads") ? `${serverOrigin}${avatarPath}` : avatarPath;
  ...
  const [filterType, setFilterType] = useState<"all" | "yesno" | "rating">("all");
  ...
  useEffect(() => {
    ...
    if (storedToken && storedUser) {
      ...
      // Normalize avatar URL to backend origin during dev if necessary
      if (parsedUser.avatarUrl && parsedUser.avatarUrl.startsWith("/uploads")) {
        parsedUser.avatarUrl = `${window.location.protocol}//${window.location.hostname}:4000${parsedUser.avatarUrl}`;
      }
      setUser(parsedUser);
      const storedAnswers = localStorage.getItem(`pollAnswers-${parsedUser.username}`);
      if (storedAnswers) {
  ...
  }, [token, user?.role, adminPage, searchTerm, filterType, filterStatus, sortOrder]);
  ...
  const login = async () => {
    ...
    if (loggedUser.avatarUrl && loggedUser.avatarUrl.startsWith("/uploads")) {
      loggedUser.avatarUrl = `${window.location.protocol}//${window.location.hostname}:4000${loggedUser.avatarUrl}`;
    }
    setUser(loggedUser);
    localStorage.setItem("token", data.token);
    localStorage.setItem("user", JSON.stringify(data.user));
    const storedAnswers = localStorage.getItem(`pollAnswers-${data.user.username}`);
    if (storedAnswers) {
      setPollAnswers(JSON.parse(storedAnswers));
    } else {
      setPollAnswers({});
  ...
  const register = async () => {
    ...
    const data = await response.json();
    // registration returns user with avatarUrl; normalize if needed
    const regUser = data;
    if (regUser.avatarUrl && regUser.avatarUrl.startsWith("/uploads")) {
      regUser.avatarUrl = `${window.location.protocol}//${window.location.hostname}:4000${regUser.avatarUrl}`;
    }
    setUser(regUser);
    setToken(null);
    setPassword("");
  ...
  const answeredPolls = surveys.filter((survey) => Boolean(pollAnswers[survey.id])).length;
  ...
  const pagedSurveys = filteredSurveys.slice((safeUserPage - 1) * limit, safeUserPage * limit);
  const pageTitle = route === "/profile" ? "Mi perfil" : "Encuestas disponibles";
  const pageSubtitle = route === "/profile" ? "Actualiza tus datos personales" : `Bienvenido${displayName ? `, ${displayName}` : ""}`;
        ...
        <div className="modal-card" onClick={(e) => e.stopPropagation()}>
          <div className="modal-header">
            <div>
              ...
              <p>{editingSurvey ? "Ajusta los datos y guarda los cambios." : "Crea una nueva encuesta para tu campus."}</p>
            </div>
          ...
          <form className="admin-form" onSubmit={saveSurvey}>
            <div className="field-row">
              ...
              <label>
                ...
                <select
                  value={pollType}
                  onChange={(e) => setPollType(e.target.value as "yesno" | "rating")}
                >
            ...
            <div className="field-row modal-actions-row">
              ...
              <div className="button-row">
                <button type="submit">{editingSurvey ? "Actualizar" : "Crear"}</button>
  ...
  const submitResponse = async (survey: Survey) => {
    ...
    try {
      ...
      setPollAnswers((prev) => {
        ...
        if (user) {
          localStorage.setItem(`pollAnswers-${user.username}`, JSON.stringify(next));
        }
        return next;
      });
    } catch {
  ...
  const loadProfileForm = () => {
    ...
    setProfileAvatarUrl(user.avatarUrl || "/default-avatar.svg");
  };
  ...
  const updateProfile = async () => {
    ...
    try {
      ...
      // Normalize avatar path returned by server
      if (updated.avatarUrl && updated.avatarUrl.startsWith("/uploads")) {
        updated.avatarUrl = `${window.location.protocol}//${window.location.hostname}:4000${updated.avatarUrl}`;
      }
      setUser(updated);
    ...
    return (
      <div className="container">
        ...
        <section className="box login-box">
          ...
          <div className="input-group">
            <label className="input-with-icon">
              ...
              <button
                ...
                aria-label={showPasswordField ? "Ocultar contraseña" : "Mostrar contraseña"}
              >
                {showPasswordField ? (
          ...
          {successMessage && (
            <p className={`success-message${successVisible ? " visible" : ""}`}>{successMessage}</p>
          )}
          {error && <p className="error">{error}</p>}
          <p>
            No tienes cuenta? <button type="button" className="link-button" onClick={() => navigate("/register")}>Regístrate</button>
          </p>
        </section>
      </div>
    );
    ...
    return (
      <div className="container">
        ...
        <section className="box">
          ...
          <button type="button" className="secondary" onClick={() => navigate("/login")}>Volver al login</button>
          {error && <p className="error">{error}</p>}
        </section>
      </div>
    );
    ...
    <section className="box profile-box">
      ...
      <div className="profile-avatar-row">
        <div className="profile-avatar-preview">
          <img src={profileAvatarUrl || "/default-avatar.svg"} alt="Avatar preview" />
        </div>
        <div className="profile-avatar-controls">
          <label className="small">Subir imagen</label>
          <input type="file" accept="image/*" onChange={handleAvatarFile} />
          <p className="muted">O pega una URL en el campo Avatar URL más abajo.</p>
        </div>
      </div>
      <label>
        Nombre completo
      ...
      <label>
        Nombre de usuario
        <input value={profileUsername} onChange={(e) => setProfileUsername(e.target.value)} />
      </label>
      <label>
        Correo
      ...
      <div className="modal-overlay" onClick={closePasswordModal}>
        <div className="modal-card" onClick={(e) => e.stopPropagation()}>
          <div className="modal-header">
            ...
            <button type="button" className="modal-close" onClick={closePasswordModal}>
              ×
            </button>
          </div>
  ...
  const renderAdminPanel = () => (
    <section className="box admin-box">
            ...
            <article key={survey.id} className="admin-card">
              <div className="admin-card-header">
                <div className="admin-card-title">
                  ...
                  <p className="poll-updated">Última actualización: {formatUpdatedAt(survey.updatedAt)}</p>
                </div>
                <div className="pill-group">
                  <span className={`pill ${survey.active ? "active" : "inactive"}`}>
                    {survey.active ? "Activa" : "Inactiva"}
                  </span>
                  <span className={`pill ${survey.type === "rating" ? "rating" : "active"}`}>
              ...
              {survey.summary && (
                <div className="admin-card-summary">
                  ...
                  {survey.type === "yesno" ? (
                    ...
                    <div className="summary-item">
                      <span>Promedio</span>
                      <strong>{survey.summary.average?.toFixed(1) ?? "0.0"}</strong>
                    </div>
                  )}
                </div>
              )}
              <div className="admin-actions">
                ...
                <button type="button" className="danger" onClick={() => deleteSurvey(survey.id)}>
                  Eliminar
                </button>
              </div>
            </article>
      ...
      <div className="pagination">
        <button type="button" disabled={adminPage <= 1} onClick={() => setAdminPage(adminPage - 1)}>
          Anterior
        </button>
        <span>Página {adminPage} de {totalPages}</span>
        <button type="button" disabled={adminPage >= totalPages} onClick={() => setAdminPage(adminPage + 1)}>
          Siguiente
        </button>
      </div>
      {renderModal()}
  ...
  const renderMainSection = () => {
    ...
    return (
      <section className="box">
              ...
              <article key={survey.id} className="survey-card">
                <div className="survey-card-header">
                  ...
                  <div className="survey-card-meta">
                    ...
                    <span className="poll-updated">{formatUpdatedAt(survey.updatedAt)}</span>
                  </div>
                </div>
                {hasAnswered && activePollId !== survey.id && (
                  <div className="survey-card-answer">
                    <span className="pill answered">Respondida</span>
                    <p className="previous-answer">Tu respuesta: {previousAnswer}</p>
                  </div>
                )}
                <div className="poll-actions">
                  {activePollId === survey.id ? (
                    <div className="poll-form">
                      ...
                      <div className="button-row">
                        <button type="button" onClick={() => submitResponse(survey)}>
                          Enviar voto
                        </button>
                        <button
                          type="button"
        ...
        {filteredSurveys.length > 0 && (
          <div className="pagination">
            <button type="button" disabled={userPage <= 1} onClick={() => setUserPage(userPage - 1)}>
              Anterior
            </button>
            <span>Página {safeUserPage} de {userTotalPages}</span>
            <button type="button" disabled={userPage >= userTotalPages} onClick={() => setUserPage(userPage + 1)}>
              Siguiente
            </button>
          </div>
        )}
  ...
  return (
    <div className="app-shell">
      ...
      <aside className={`sidebar ${sidebarOpen ? "open" : "closed"}`}>
        ...
        {user?.role !== "admin" && (
          <>
            <div className="activity-badge">{answeredPolls} encuestas respondidas</div>
            <div className="activity-badge secondary">{unansweredPolls} encuestas sin responder</div>
          </>
        )}
        <nav className="nav-bar">
          <button type="button" className={route === "/" ? "active" : ""} onClick={() => navigate('/')}>Home</button>
          <button type="button" className={route === "/profile" ? "active" : ""} onClick={() => navigate('/profile')}>Perfil</button>
        </nav>
        ...
        <div className="sidebar-user-panel">
          <div className="avatar">
              {(() => {
                ...
                const src = raw && raw.startsWith("/uploads") ? `${backendOrigin}${raw}` : (raw || "/default-avatar.svg");
                return <img src={src} alt={`${displayName} avatar`} />;
              })()}
            </div>
